...
export function ImportDialog({ summary, onCancel, onConfirm }: Props) {
  ...
  return createPortal(
    ...
    >
      ...
      >
        <header className="px-6 py-4 border-b border-slate-100 flex items-start justify-between gap-3">
          <h2 className="text-base font-semibold text-slate-900">{t('importDialogTitle')}</h2>
          <button
            onClick={onCancel}
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-900">{t('importMerge')}</div>
                  <div className="text-xs text-slate-500 mt-0.5">{t('importMergeDesc')}</div>
                </div>
              </label>
              <label
                className={`flex items-start gap-2.5 p-3 rounded-lg border cursor-pointer transition-colors ${
              ...
              >
                ...
                />
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-900">{t('importReplace')}</div>
                  <div className="text-xs text-slate-500 mt-0.5">{t('importReplaceDesc')}</div>
                </div>
              </label>
            </div>
          </div>
        ...
        <footer className="px-6 py-3 border-t border-slate-100 flex justify-end gap-2 bg-slate-50/40">
          <button
            onClick={onCancel}
